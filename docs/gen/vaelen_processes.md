...
The present-day author is wind. Dunes, yardangs, mantled crater rims, and
polished plains should overprint older impact and hydrology features. The
world should feel useful and reachable, but also lonely: a frontier built on
borrowed time.

A single Olympus-class shield volcano anchors the body as a recognizable
landmark — a very gentle dome roughly 600 km across with a basal scarp cliff,
a summit caldera, and radial flow lobes down its flanks. From orbit it should
read as the obvious "thing" on Vaelen.
...
- linear albedo and roughness from per-biome palette functions of physical
  fields (altitude-in-biome, slope, curvature, feature proximity)
- analytic feature buffers for craters, channels, rifts, dunes, and rocks
...
2. Crustal memory: highland swells, long rifts/scarps, broad volcanic plains,
   one Olympus-class shield volcano as the body's primary landmark.
...
5. Present desert: dust mantle, dune seas, yardangs, mantled crater rims.
...
1. **Anchor biomes:** place 2-4 large, asymmetric visual anchors: at least one
   pale ancient basin, one dark volcanic or impact province, and one rusty
   highland/dust province with a strong boundary or scarp. Each carries a
   palette function of physical fields (altitude / slope / curvature) so its
   color comes from its topography rather than from a separate marbling layer.
2. **Shield landmark:** place one Olympus-class shield volcano as the body's
   primary recognizable feature. Its flanks become their own biome with
   `RadialFlow` height generation; its basal scarp is structural.
3. **Ancient wet story:** make pale basins structurally meaningful. They should
...
4. **Dark contrast:** strengthen dark basaltic and impact-melt terrain enough
   to prevent a one-note orange planet, but keep it patchy and geologically
   old. It should feel partly buried or dust-mantled.
5. **Wind overprint:** add dune seas and mantled crater rims after the older
   features exist. Dunes should pool and spill around rough terrain, not
   appear as procedural wallpaper.
6. **Surface usability:** preserve broad landing-friendly plains in basin
...
This order keeps the generator aligned with the lore: crust, shield landmark,
and impacts create old structure, ancient water modifies it, and the current
desert reworks the surface.
...
- dark volcanic, pale sediment/evaporite, and rust highland biomes with palette
  functions of (altitude, slope, curvature)
- one shield volcano with basal scarp and `RadialFlow` flanks
- one Valles-class rift
- a few seeded canyon/channel systems
- one dune sea with wind-aligned ridge detail
- degraded crater population layered into the height field, density modulated
  per biome

This is not the final physical model, but it establishes the correct visual
contract: no visible polygon cells, no categorical debug paint, no separate
marbling field, and a model that can grow toward authored terrain at every
scale.
