...
- Terrain collision is one `Kinematic` trimesh patch whose vertices are
  ...
  [surface.md](surface.md) §3.6 (the **Landing & impact destruction** part).
...
> **Naming clash with `surface.md`.** That doc uses "surface
...
- **EVA landed → Surface:** EVA isn't in `BodyFixed` yet
  (surface.md §2.2/§4.2); the `altitude < ceiling` term still
...
- Landed-EVA Surface becomes exact once the EVA→BodyFixed migration
  (surface.md §4.2) lands; until then the `altitude < ceiling`
  term already selects Surface for low EVA.


## Background — layered astrodynamics methods (Kepler / Encke / Cowell / Avian)
