...
>    (SpeedTree "puffiness"). Three crown shapes (`CanopyStyle`): the **broadleaf**
>    (`push_branched_canopy`) grows a recursive **branched limb** structure with a
>    foliage cluster at each tip, so the crown spreads asymmetrically like a real
>    tree; **shrubs** (`Round`, `push_canopy`) are a Fibonacci-skin + lobe ellipsoid
>    clump; **conifers** (`push_conifer`) are a conical scatter of drooping
>    needle-spray cards. Cards/limbs/trunk sample the procedural **foliage atlas**
...
> cast roughly-solid (un-dappled) shadows; the octahedral impostor is still baked
> from vertex-colour (no leaf alpha), so the far band doesn't yet match the new
> near look; no atlas mips (far cards may shimmer); conifer cones use needle-card
> texturing but aren't yet drooping needle cards; no real branch structure. See
> the "Realistic trees" slice plan.
