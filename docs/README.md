...
`AGENTS.md` is a pointer to it (Cursor loads both filenames). Two skills sit
beside it: `steer` (`.claude/skills/steer/SKILL.md`) starts from `just queue`,
and `diag-triage` (`.claude/skills/diag-triage/SKILL.md`) runs the diagnostics
pass. Historical adversarial-review reports live in [reviews/](reviews/); that
harness is retired until the skill exists again.
