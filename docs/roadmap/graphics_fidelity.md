...
| W18 | Contact shadows + PCSS contact-hardening — **the contact + mid-field tiers** of the three-tier split | ◐ | Med | verify | **a/c and the temporal-coherence repair landed 2026-07-29** (`just check thalos_game` clean): contact shadows now march the current Bevy depth prepass before opaque shading (single-sample + MSAA) and feed the default tile material in the same frame; cascades overlap/cross-fade; PCSS uses a deterministic disk, physical solar angular radius, and crisp 2×2 contact kernel. The whole cascade state is now a PostUpdate→Last transaction, with continuous sun/basis and smoothed footprint scale; `stability_gauge` + `just diag` detect render-origin frame drift. **Runtime temporal verdict pending.** `THALOS_CONTACT_SHADOW=off/show` + the `shadow` compare axis. **b remains optional optimization:** hardware `sampler_comparison` can replace hand comparisons if profiling justifies the binding churn. Far-field remains W12. · ADR-20260722T111848Z · INC-20260729T051825Z |

## 4.3 Ambient occlusion & GI

| ID | Item | Status | Effort | Sprint | Notes / source |
|---|---|---|---|---|---|
| W8 | **Visibility-Bitmask AO (VBAO)** → `ThalosSurface.occlusion` + Bevy ambient | ◐ | Med | THIS | Therrien 2023 / XeGTAO. **First cut landed as F5 (2026-07-01):** custom half-res **hemisphere SSAO** node (`rendering::ssao`) reading the foundation-owned `SceneDepthImage`, viewspace-only (f32-safe), small world radius, into the terrain `surf.occlusion`. **Not yet VBAO** — it's classic hemisphere SSAO (no bitmask/thickness heuristic for thin grass/trunks); upgrading the shader to VBAO is a drop-in follow-up. **Remaining:** veg/rock/hull materials + Bevy StandardMaterial ambient + a spatial blur (currently IGN-rotated). Delivers one-world invariant #4 (mutual contact AO) for the dominant surface |
