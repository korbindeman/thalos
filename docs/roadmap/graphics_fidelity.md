...
| **F7** | Metallic conductor branch in `surface_brdf` + **one shared view-level scene+atmosphere bind group** + roughness-mip prefiltered env from the F3 LUT. **The bind group now also carries W11's aerial-perspective froxel volume** — binding the volume into 8 materials before F7 means rewiring all 8 after, so the two ship as one slice (ADR-20260722T111847Z). | ☐ | High | next |
...
| W12 | **Terrain horizon-angle self-shadow** sampled by *all* surfaces (mountain shadows valley + objects) — **the far-field tier** of the three-tier shadow split; cascades structurally cannot reach here (ADR-20260722T111848Z) | ◐ | Med-High | next | Terrain-side already ships (`terrain_self_shadow` height-atlas march in `body_terrain.wgsl`, bounded by resident tiles). **Object-side v1 landed 2026-07-02:** `body_render::horizon_sun_visibility` (pure f64 body-local march of the body's `HeightSource` along the sun azimuth, ~30 km reach, no bake) evaluated at the craft each frame in `update_sun_light` and multiplied into the sun `DirectionalLight` illuminance — a mountain between the low sun and the parked craft/base now pulls the direct term on hull/structures/EVA to 0 (ambient sky fill deliberately kept). **Remaining for full W12:** per-fragment horizon term for trees/grass/rocks (spine materials), longer-than-resident-tile terrain reach, max-mip acceleration |
| W18 | Contact shadows + PCSS contact-hardening — **the contact + mid-field tiers** of the three-tier split | ◐ | Med | **THIS** | **a landed 2026-07-22** (check + clippy clean, awaiting screenshot): `rendering::contact_shadow` — a full-res R16Float pass over `SceneDepthImage`, 12-step view-space march toward the sun, thickness-tested occluders, grazing-widened normal bias, 120 m fade-out. Full-res and unblurred on purpose (SSAO's half-res + blur would erase exactly the high-frequency edge this produces). Gate rides `ShadowCascadeBlock.gate.z`, published by the rig so all consumers inherit it; `thalos::shadow::contact_shadow_factor` is the one sampler. Terrain-only receiver for now (F5's staging); `THALOS_CONTACT_SHADOW=off/show` + the `shadow` compare axis. **b/c pending.** · ADR-20260722T111848Z-shadows-three-tier-not-virtual-shadow-maps. Shadows are three range regimes, not one: **contact** (0–50 m — cascade 0 is ~0.2 m/texel, coarser than a gear strut), **mid-field** (50 m–5 km — what cascades are for), **far-field** (5 km–horizon — a 100 km box at 4096² is 24 m/texel; that's W12's horizon term, not a cascade). **a:** screen-space contact shadows, short depth march reusing `SceneDepthImage` + F5's SSAO node pattern — grounds objects, fixes peter-panning, highest value-per-effort on this table. **b:** `cascade_factor` → hardware `sampler_comparison` (16 `textureLoad`s + hand tent → 9 `textureSampleCompare` taps) — funds **c:** PCSS on cascades 0/1 + a cross-cascade blend (only the outermost fades today). **Cascade extents do not grow.** VSM rejected — see the ADR |
...
[clouds.md](../rendering/clouds.md). This table keeps the stable W-IDs, but cloud design,
...
| — | Object aerial recession (foliage/rocks → sky haze) | ◐ | — | THIS | `object_aerial_recession`. **Superseded by W11 — it gets deleted, not folded in.** It is a distance `smoothstep(1 km, 35 km)` lerp toward `sky_radiance` with no transmittance (so distant darks can't lose contrast and `OBJECT_AERIAL_BRIGHTEN_CAP` exists only to stop white-out), no sun direction (no Mie forward-scatter glare — the dominant MSFS cue), and no altitude term (same curve at sea level and at `cruise`). Its 5 callers (grass/gpu_grass/rock/tree/tree_impostor) recede differently from the terrain behind them, which shades through the raymarch — one-world invariant #5 is violated *today* |
| W11 | Hillaire aerial-perspective **froxel** volume | ☐ | High | **next** | ADR-20260722T111847Z-aerial-perspective-froxel-volume-regime-split. 32³ RGBA16F view-space LUT (RGB in-scatter, A transmittance), exponential slices, built from the **same** `integrate_atmosphere_multiscatter` primitives — a reorganisation of the existing model into a lookup, never a second model. **Regime split is the load-bearing decision:** the volume covers only the in-atmosphere near range (far plane = `min(atmosphere exit, ~128 km)`), the raymarch stays authoritative beyond it, and above the Kármán line the volume is **off** entirely (an exponential distribution would spend ~30 of 32 slices in vacuum). Every opaque surface applies `color·T + L`; terrain, hull, structures, and ocean join the 5 vegetation callers on one model. 3-channel transmittance for sunset. Cost is resolution-independent; view-space build is origin-relative, so precision *improves* under big_space. **Ships in F7's slice** (the shared view-level bind group). **b:** `thalos::shadow` per froxel → volumetric shafts, near-free once the march exists — the reason to do W11 before further shadow-filter work. Acceptance needs BL-37's `aerial` axis: both handoffs are new artifact surfaces and the volume will look plausible while being energy-wrong |
