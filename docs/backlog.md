...
| MIRA-1 | **Airless diffusion patch proof:** Rust/Burn 0.21 backend-generic denoiser + DDPM/DDIM contract; deterministic labelled crater/ejecta/gardening corpus; physical-scale Laplacian S0–S3; SafeTensors + canonical tensor hash; real per-batch EMA; epoch checkpoints with raw/EMA weights + Adam + path-remapped Burn parameter IDs (resumed epoch 3 exactly matched uninterrupted hashes/loss); shared-noise overlapping-window sampler; three exact CC0 Kaguya S3 artifacts plus three bounded 128 ppd SLDEM2015 macro ranges in disjoint train/Copernicus-validation/Tycho-holdout blocks; checksum-gated Rust GeoTIFF/PDS FLOAT decoding, resampling, inpainting, patch extraction, indices, and inspected hillshades. Remaining: SLDEM distribution-rights confirmation, combined real+synthetic S0–S3 campaign quality, spectral/slope/SFD + VRAM evidence | wip | L–XL research | MIRA-0 schema | mira_airless_mvp.md §6, §8 · ADR-0010 |
