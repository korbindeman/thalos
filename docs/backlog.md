...
| MIRA-1 | **Airless diffusion patch proof:** Rust/Burn 0.21 backend-generic denoiser + DDPM/DDIM contract; deterministic labelled crater/ejecta/gardening corpus; physical-scale Laplacian S0–S3; SafeTensors + canonical tensor hash; real per-batch EMA; epoch checkpoints with raw/EMA weights + Adam + path-remapped Burn parameter IDs (resumed epoch 3 exactly matched uninterrupted hashes/loss); shared-noise overlapping-window sampler; three exact CC0 Kaguya S3 artifacts plus three bounded 128 ppd SLDEM2015 macro ranges in disjoint train/Copernicus-validation/Tycho-holdout blocks; checksum-gated Rust GeoTIFF/PDS FLOAT decoding, resampling, inpainting, patch extraction, indices, and inspected hillshades. Remaining: SLDEM distribution-rights confirmation, combined real+synthetic S0–S3 campaign quality, spectral/slope/SFD + VRAM evidence | wip | L–XL research | MIRA-0 schema | mira_airless_mvp.md §6, §8 · ADR-0013 |
...
| CLOUD-1 | Canonical cloud ownership/schema: one authored `CloudClimate`, one per-body runtime `CloudWeatherField`, `None` authoritative, first weather-derived orbit layer; absorb render mechanism under `body_render` and delete legacy/reference ownership | done | M–L | compile + warning-clean; all five headless cloud regimes captured without asset/WGSL/pipeline failure 2026-07-20 | ADR-0009 · cloud §2–§4 |
| CLOUD-2 | Scalable targets and temporal reconstruction: viewport-relative/interleaved march, history rejection/clamp/upscale, screenshot mode and quality ladder | done | L | completed 2026-07-21: viewport-relative 1/2–1× targets, rotating 3×3 sparse march, body-fixed reprojection with cut/depth/epoch rejection, 3×3 neighborhood clamp, bilinear history + hit-aware upscale; 1440p High sunset 3.35 ms mean / 3.36 p95 at 1712×960 (≤3.5 ms target) | cloud §3.3 · §4 · cloud_baseline.md |
| CLOUD-3 | Multi-scale density and range: typed vertical profiles, true 3-D base/detail noise, anti-tiling, cadence-safe range LOD | done | L | corrected 2026-07-21 after matched A/B: heuristic hierarchy leaps caused posterized height strata and were deleted (ADR-0012); proven typed 3-D density restored, fine erosion fades 10–22 km, 21.6 km macro modulation is reused per ray. Five views clean; 1440p High sunset 2.471 ms mean / 2.476 p95 | cloud §3.2–§3.3 · INC-0010 · INC-0011 · cloud_baseline.md |
| CLOUD-4 | Atmosphere-coupled cloud lighting and correct foreground/background media ordering | wip | L | first slice 2026-07-21: analytic sun/view air-mass transmittance, multi-scatter phase + powder, energy peak, cooler orbital albedo; residual: shared atmosphere LUT bind, explicit FG/BG media split in BodySky composite | cloud §3.4 |
...
| CLOUD-6 | Density-derived orbital projection and invisible surface/orbit/map handoff | wip | M–L | first slice 2026-07-21: shared weather-column moments (coverage/type/base/top → optical depth + height), soft cube fetch, height-parallax shell, height-moment normals + self-shadow on SolidPlanet + BodySky; limb probe shows softer broken systems without cubemap confetti; residual: offline OD/normal atlas, reduced-detail limb volume, motion handoff verify | cloud §3.6 |
