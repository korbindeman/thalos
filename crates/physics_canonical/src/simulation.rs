...
pub struct SimulationConfig {
    ...
    /// Minimum altitude above the dominant body's mean radius required to
    /// engage each warp level, expressed as a fraction of the body's
    /// radius_m. Same length and order as `warp_levels`; index `i` is the
    /// minimum altitude (in body-radii) needed to use `warp_levels[i]`. A
    /// value of `0.0` means the level is always available. Game-side
    /// enforcement is in `crate::bridge`; canonical simulation does not
    /// read this on the hot path. Scaling by radius keeps the same table
    /// usable across all bodies in the system.
    pub warp_min_altitude_radii: Vec<f64>,
...
impl Default for SimulationConfig {
    fn default() -> Self {
        Self {
            ...
            // Fraction of body radius required for each level. Sized so that
            // a frame's Kepler advance at the given multiplier cannot punch
            // through the body even at orbital speed: at 60 FPS one frame is
            // `0.0167 * w` seconds, and worst-case low-orbit velocity is
            // ~`sqrt(g_surface * r) ≈ sqrt(μ/r)`. Numbers here are conservative
            // multiples of that distance.
            warp_min_altitude_radii: vec![
                0.0,   // 0×   — paused
                0.0,   // 1×   — live
                0.001, // 10×  — no warp above 1× in the surface zone
                0.001, // 100× — ~3 km on Thalos / ~250 m on a 250 km body
                0.01,  // 1k×
                0.05,  // 10k×
                0.25,  // 100k×
                1.0,   // 1M×
...
pub struct WarpController {
    ...
    level_index: usize,
    levels: Vec<f64>,
    /// Minimum altitude in body-radii required to engage each level. Same
    /// length and order as `levels`. Read by the game-side enforcement
    /// system; canonical step does not consult this on the hot path.
    min_altitude_radii: Vec<f64>,
...
impl WarpController {
    fn new(levels: Vec<f64>, min_altitude_radii: Vec<f64>) -> Self {
        let speed = levels.first().copied().unwrap_or(0.0);
        // Pad / truncate the min-altitude table so callers don't have to
        // worry about length mismatch. Missing entries fall through to a
        // "free" 0.0 requirement.
        let mut min_alts = min_altitude_radii;
        min_alts.resize(levels.len(), 0.0);
        Self {
            level_index: 0,
            levels,
            min_altitude_radii: min_alts,
            resume_index: None,
            speed,
    ...
    /// Minimum altitude (in body-radii) required to engage `level_index`.
    /// Returns `0.0` for any out-of-range index so callers can iterate
    /// without bounds checks.
    pub fn min_altitude_radii_for(&self, level_index: usize) -> f64 {
        self.min_altitude_radii
            .get(level_index)
            .copied()
            .unwrap_or(0.0)
    }

    /// Snap to `target_index` without a smooth transition. Used by the
    /// altitude enforcement system to clamp the active level when the
    /// craft drops below the min-altitude requirement for the current
    /// level (or when collision is detected and warp must drop to 1×).
    pub fn clamp_to_level(&mut self, target_index: usize) {
        let clamped = target_index.min(self.levels.len().saturating_sub(1));
        if clamped >= self.level_index {
            return;
        }
        self.resume_index = None;
        self.set_level_immediate(clamped);
    }
...
mod tests {
    ...
    fn ctrl() -> WarpController {
        WarpController::new(vec![0.0, 1.0, 10.0, 100.0, 1_000.0], vec![])
    }
