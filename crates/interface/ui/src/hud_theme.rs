...
/// Ordered `.after(crate::init_ui_theme)` (see `hud/mod.rs`) so the
/// shared glass material handle exists.
pub fn init_theme(
    mut commands: Commands,
    asset_server: Res<AssetServer>,
    ui_theme: Res<crate::UiTheme>,
) {
    // The palette derives from the shared design tokens (`crate::tokens`)
...
pub fn panel_frame(theme: &HudTheme) -> ((MaterialNode<GlassMaterial>, BoxShadow), BorderColor) {
    (
        (
            ...
            crate::widgets::panel::panel_shadow(),
        ),
...
/// shared kit (`crate::style_buttons` via [`crate::UiButton`]); only
    ...
    parent
        .spawn((
            ...
            Node {
                ...
                border: UiRect::all(Val::Px(1.0)),
                border_radius: BorderRadius::all(Val::Px(crate::RADIUS_CTRL)),
            ...
            BorderColor::all(crate::tokens::STROKE),
            Interaction::None,
            crate::UiButton::default(),
            binding,
        ))
        .with_children(|c| {
            c.spawn((
