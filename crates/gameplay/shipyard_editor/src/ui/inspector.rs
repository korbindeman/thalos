...
pub(super) fn rebuild_inspector(
    ...
    *shown = Some(key.clone());
    ...
    commands.entity(content_entity).with_children(|c| {
        ...
        let (factor, suffix) = thalos_game_state::units::format::length_display(system);
        let len = F::Scaled {
...
pub(super) fn update_info_text(
    ...
    let d = |v: f32| thalos_game_state::units::format::length(v as f64, 2, system);
    let kg = |v: f32| thalos_game_state::units::format::mass_large(v as f64, system);
    let a = |v: f32| thalos_game_state::units::format::area(v as f64, system);
    ...
    if let Some(p) = pod {
        ...
        lines.push(format!(
            "ejection {}",
            thalos_game_state::units::format::impulse(dec_part.ejection_impulse as f64, system)
        ));
        ...
        lines.push(format!(
            "Ø{} · {} · Isp {:.0} s (fixed)",
            d(e.diameter),
            thalos_game_state::units::format::thrust(e.thrust as f64, system),
            e.isp
        ));
