...
use crate::core::{CollectQuery, EditorPart, EditorState, collect_blueprint};
use thalos_shipyard::flyability::FlyabilityFinding;
use thalos_shipyard::{
...
pub struct EditorStatsCache {
    ...
    /// Why this build cannot fly, worst first. Empty is the healthy
    /// case. `LAUNCH` refuses while any finding is `Blocking`, so the
    /// player learns in the VAB rather than at 40 km.
    pub flyability: Option<Result<Vec<FlyabilityFinding>, String>>,
...
fn refresh_stats_cache(
    ...
    cache.flyability = blueprint
        .as_ref()
        .map(|bp| bp.flyability(&catalog).map_err(|e| e.to_string()));
