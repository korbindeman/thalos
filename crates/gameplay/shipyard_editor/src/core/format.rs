...
//! [`thalos_game_state::units::format`], the one place SI becomes a display string, so the
...
//! [`thalos_game_state::units::UnitDomain::General`] — it follows the global switch and never the
