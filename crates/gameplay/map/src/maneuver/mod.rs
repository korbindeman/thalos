...
fn update_camera_block(
    ...
    mut block: ResMut<thalos_game_state::camera::BlockCameraInput>,
) {
...
fn update_selected_node_view(
    ...
    scale: Res<thalos_game_state::coords::WorldScale>,
...
impl Plugin for ManeuverPlugin {
    fn build(&self, app: &mut App) {
        app.add_message::<ManeuverEvent>()
            ...
            .add_systems(
                Startup,
                setup_maneuver_editor.after(thalos_ui::hud_theme::init_theme),
            )
            ...
            .add_systems(
                ...
                end_drag_on_release
                    ...
                    .before(thalos_game_state::sched::SimStage::Physics),
            )
            .add_systems(
                Update,
                (
