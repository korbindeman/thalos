...
impl Plugin for BodyTreePanelPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<BodyTreeState>()
            .add_systems(Startup, setup_ui.after(thalos_ui::hud_theme::init_theme))
            .add_systems(
                Update,
                (
                    update_visibility,
...
fn update_visibility(
    ...
    photo: Res<PhotoMode>,
    context: Option<Res<State<thalos_game_state::context::GameContext>>>,
    mut roots: Query<&mut Visibility, With<BodyTreePanelRoot>>,
) {
    let editor_open = context
        .as_deref()
        .map(|c| !matches!(*c.get(), thalos_game_state::context::GameContext::Flight))
        .unwrap_or(false);
    let visible =
