...
//! crate, `thalos_runtime::local_physics::*`, `thalos_runtime::aero` (force layer),
//! `thalos_runtime::regime` (authority executor), and the documented EVA exception
...
use bevy::prelude::*;
use thalos_terrain::{HeightSource, TerrainPatchBasis, TerrainPatchConfig};
...
/// Bevy-side registry of the shared, renderer-independent terrain-height
/// contract. Renderer-specific residency mirrors are composed separately by
/// `thalos_runtime`.
...
/// by the `AvianOwnership` predicate in `crates/runtime/game/src/local_physics.rs`:
