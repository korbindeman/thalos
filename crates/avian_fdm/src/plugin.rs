...
pub struct AircraftFdmPlugin {
    ...
    /// Register the built-in [`update_atmosphere`] system, which derives ISA
    /// [`AtmosphereState`](crate::components::AtmosphereState) from the root's
    /// world-space Y altitude. Default: `true`.
    ///
    /// **Thalos sets this to `false`** (vendored-fork addition): its aircraft
    /// live in a body-centered-inertial bubble where world-Y is not altitude,
    /// and the atmosphere is a per-body density model, not Earth ISA. With this
    /// off, the consumer must write `AtmosphereState` each frame in (or before)
    /// the [`AircraftFdmSystems::Atmosphere`] set so `FlightState`/`Forces` read
    /// a populated atmosphere.
    pub manage_atmosphere: bool,
}
...
impl Plugin for AircraftFdmPlugin {
    fn build(&self, app: &mut App) {
        ...
        register_fdm_systems(app, self.manage_atmosphere);
...
fn register_fdm_systems(app: &mut App, manage_atmosphere: bool) {
    app.configure_sets(
        PhysicsSchedule,
        (
    ...
    // Vendored-fork addition: the built-in ISA/world-Y atmosphere is optional so
    // a consumer (Thalos) can own `AtmosphereState` from a per-body density model
    // in the `Atmosphere` set instead. See `AircraftFdmPlugin::manage_atmosphere`.
    if manage_atmosphere {
        app.add_systems(
            PhysicsSchedule,
