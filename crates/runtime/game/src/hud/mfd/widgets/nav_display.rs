...
//! computed by [`crate::route`] — is drawn as a real polyline with its final
...
//! send a [`RouteRequest`], so [`crate::route`] stays the sole writer and the
...
//! built exactly like the shared [`crate::hud::geo::local_enu_basis`], so ND and
