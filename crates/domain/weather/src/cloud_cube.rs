//! The canonical per-body cloud **weather cube** — producer side.
//!
//! `CloudWeatherField` is the mutable large-scale volumetric-cloud authority a
//! body carries at runtime: RGBA8 cubemap payloads for coverage / cloud type /
//! base / top plus the four-stratum broad-density shape cube, derived once
//! from the authored [`thalos_world::CloudClimate`] (running the shallow-water
//! [`WeatherSim`](crate::WeatherSim) for the synoptic pattern). Every render
//! projection — near tier, orbital tier, cloud shadows — consumes this one
//! field; no projection owns an independent pattern
//! (ADR-20260720T212214Z-one-weather-field-many-cloud-projections).
//!
//! Moved here from `thalos_runtime::solar_system_state` (Phase 5a,
//! ADR-20260731T024003Z): the derivation is weather-domain logic and iterating
//! on it must not rebuild the game runtime. Pure Rust — glam math only, no
//! Bevy.
...
use crate::{WeatherSim, WeatherSimParams};

/// `bevy::math::FloatExt::lerp` equivalent for this pure crate.
#[inline]
fn lerp_f32(a: f32, b: f32, t: f32) -> f32 {
    a + (b - a) * t
}

/// 1.0 = trust the weather field; global coverage trim.
///
/// This is the ONE definition (the renderer re-exports it). The CPU strata
/// producer ([`cloud_surface_density_traced`]) and the renderer's
/// fill-calibration input both read it from here — it used to be hand-copied
/// into four places, so trimming coverage moved the near tier and the far tier
/// by different amounts and the two stopped agreeing on how cloudy the planet
/// was.
///
...
pub const COVERAGE_SCALE: f32 = 1.0;
        ...
        for face in CubemapFace::ALL {
            for y in 0..face_size {
                ...
                for x in 0..face_size {
                    ...
                    let cloud_type = lerp_f32(deck_type, 0.04, veil_share).clamp(0.02, 0.97);
                    ...
                    let base = lerp_f32(deck_base, veil_base, veil_share);
                    let top = lerp_f32(deck_top, veil_top, veil_share).max(base + 0.04);
                    ...
                    let amplitude = lerp_f32(1.0, 0.26, veil_share);
                    ...
                    if trace_stride > 0 && texel_index.is_multiple_of(trace_stride) {
...
pub fn cloud_surface_density_traced(
    ...
    let cov = (coverage * COVERAGE_SCALE).clamp(0.0, 1.0);
