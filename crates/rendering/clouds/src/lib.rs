//! Volumetric-cloud compute mechanism (Thalos fork of `bevy-volumetric-clouds`,
//! MIT, evroon — see `README.md` and `UPSTREAM_LICENSE`).
//!
//! This crate owns the geometry-agnostic half of the cloud renderer: generated
//! Perlin-Worley noise, the HZD density+lighting raymarch, planet-fixed weather
//! and strata cubes, and the sun-transmittance cascade. Applications write
//! [`CameraMatrices`] and [`CloudsConfig`] each frame in the **body-fixed
//! frame** of the cloud body, then composite [`CloudRenderTexture`] over their
//! own scene.
//!
//! The planetary `BodySky` composite — orbital far-tier, analytic-ocean hit,
//! atmosphere block — stays in `thalos_body_render`. Kòrsou composites only the
//! near volume against copied scene depth.
...
/// in-scatter, `a` = transmittance). The consumer binds this in a fullscreen
...
/// coverage, cloud type, normalized base, normalized top.
#[derive(Resource, Clone)]
...
/// [`CloudShadowFrame::block`] packs for them.
///
/// **Sole writer:** the application's cloud driver.
#[derive(Resource, Clone)]
pub struct CloudShadowMap {
    pub handle: Handle<Image>,
    /// Where the cascade was marched, body-fixed.
    pub frame: CloudShadowFrame,
    /// World render space → the marcher's body-fixed frame.
...
/// crate does not auto-derive it from a camera entity).
...
impl Plugin for CloudsPlugin {
    fn build(&self, app: &mut App) {
        ...
        // file, so the crate rename is irrelevant).
