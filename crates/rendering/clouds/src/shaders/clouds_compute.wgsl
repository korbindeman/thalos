// Scattering terms are shared with every other participating medium in the
// game (vapour cones, fog, dust) — see `thalos::volumetrics`. The MARCH below
// stays cloud-specific on purpose: planetary shell, coverage-calibrated fill
// LUT, tiering and temporal amortization are not shareable with a
// twenty-metre attached volume. Share the terms, keep the loop local.
// Functions only: naga_oil does not carry `const` across a module boundary.
...
struct Config {
    ...
    // Sim-time seconds driving `cloud_cell_field`'s per-octave drift (cells
    // growing and dissolving). Must match the composite's `cloud_time.x` or
    // the two tiers render different cloud shapes at the same instant.
    cell_evolution_s: f32,
...
fn get_cloud_map_density(
    ...
    let cell = cloud_cell_field(
        up_shape, config.planet_radius, filter_m, cell_style, config.cell_evolution_s,
    );
...
fn raymarch(ray_origin: vec3f, ray_dir: vec3f, max_dist: f32, jitter: f32) -> RaymarchResult {
    ...
    for (var iteration: u32 = 0u; iteration < iteration_cap; iteration++) {
        ...
        if (clouds_density_sampled > 0.0) {
            ...
            // Single + multiple scattering for a water medium. Identical maths to
            // the inline version this replaced; it now lives in
            // `thalos::volumetrics` so the vapour cone and fog cannot drift from
            // it, and so the diffusion albedo has ONE definition.
            let scattering = volumetric_scattering(ms_lobes, tau_sun, water_cloud_albedo());
            ...
            let amb = ambient_light * ambient_occlusion(tau_sun);
