...
//! inputs plus a build-time signature of the `thalos_terrain_render` source
//! tree (see `crates/terrain/build.rs`). On `load`, the stored key is
...
pub enum LoadError {
    ...
    /// the `thalos_terrain_render` source tree has moved since the bake was
    /// produced.
    HashMismatch {
        path: PathBuf,
        expected: u64,
