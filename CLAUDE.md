...
`docs/roadmap/architecture_cleanup.md`. Guiding rules for this sprint:
...
- **Finish the in-flight unifications before adding parallel machinery.** The
  `GameContext` sub-state (docs/gameplay/ui_flow.md) and `CraftRegime`
  (docs/simulation/regimes.md) migrations exist precisely to kill boolean/state sprawl —
...
- **The one-world principle** — Thalos is one physical world; *every* surface
  ...
  opts out reads as a pasted-on cut-out. See `docs/roadmap/graphics_fidelity.md` §2.3.
...
- **Shadows** ☑ — **one shadow world landed 2026-07-02** (F6, compile-clean,
  ...
  See `docs/reference/shadow_unification_prompt.md` for the full status block.
...
- **Ocean spectral tracer (OCEAN-1)** ✅ — validated per-body `OceanState`,
  ...
  `BodyOceanMaterial` sibling of the canonical custom `BodySky` atmosphere
  (ADR-20260721T050036Z); both share one optical shader contract. Full FFT
...
- **`docs/adr/`** — the decision log: *why* things are the way they are,
  ...
  don't rewrite.
- **`docs/incidents/`** — post-mortems for fixed non-obvious bugs
  (`INC-YYYYMMDDTHHMMSSZ-slug`): evidence, hypothesis differential, root cause,
  prevention, recurrence tells. Written in the same change as the fix (see
  "Bug fixing" below).
- **Identifiers are chronological, never sequential.** Every record you mint —
  ADR, incident, backlog item — is `<KIND>-<YYYYMMDDTHHMMSSZ>-<kebab-slug>`,
  stamped with the current UTC time (`date -u '+%Y%m%dT%H%M%SZ'`); the filename
  drops the kind prefix, so lexical order is chronological order. **Never
  allocate the "next number"** — a shared counter makes parallel branches mint
  the same ID for unrelated work, and no record directory carries a
  hand-maintained index to append to. Existing `INC-0001`–`INC-0021` and
  `BL-1`–`BL-40` are frozen legacy IDs: still valid to cite, never to extend or
  renumber. Plan-doc IDs (`CL-A`…`CL-G`, `F1`–`F9`, `W`-numbers, `TM`, `C1`) are
  unaffected — one document owns them. See
  ADR-20260722T170714Z-one-chronological-identity-rule.
...
just preview              # headless procedural-object gallery → PNGs in
                          #   artifacts/visual/latest/object_preview/ (trees/shrub, grass, rocks/
...
just ui-preview           # headless UI-kit kitchen sink →
                          #   artifacts/visual/latest/ui_preview.png (every thalos_ui token/widget
                          #   over a test scene). Agents iterate on the UI by
                          #   reading the PNG. See docs/gameplay/ui.md.
just ui-preview-window    # interactive kitchen sink (hover/press/typing feel;
                          #   S = screenshot). User-run (opens a window).
just texgen               # rebuild versioned runtime vegetation atlases offline
just check                # fast type-check of thalos_game (or pass another package)
...
stable human-facing entry points (`just game`, `just screenshot`, `just preview`)
or multi-step
...
`rust-toolchain.toml` pins Rust 1.97.0, the current stable toolchain, plus the
`clippy` and `rustfmt` components. There is no checked-in Cargo backend override,
so Cargo uses its default LLVM backend on every platform. Do not add
compiler-backend override to project config unless the project deliberately
changes the stable LLVM baseline for all platforms.
...
- Keep every dev renderer lane on the **one dynamic `dev-renderer`
  ...
  budget evidence. See docs/development/build_speed.md §4.
...
For fast macro-scale inspection, use `just map`, then use the appropriate
deterministic `just screenshot <preset>` for the in-context result. There is no
separate Terrain Lab: `crates/domain/terrain` and its runtime
`ProceduralSurface` generator are the source of truth for game output.
...
(`crates/rendering/render/examples/object_preview.rs`) is a **headless** renderer:
it draws each procedural object (trees, shrub, grass, and pebbles/rocks)
to a PNG under `artifacts/visual/latest/object_preview/` and exits — no window. It renders on the
...
**Whole *game* scenes, too, you can capture headlessly** — `just screenshot`
sends a typed request to the persistent `thalos_capture_host`; `just
screenshot-cold` runs that host once and exits. The host activates
`crate::screenshot::HeadlessScreenshotPlugin` with `THALOS_SCREENSHOT` and
composes the same `thalos_runtime::AppBuilder` plugin graph as the thin
`apps/game` binary, with no window or winit (a `ScheduleRunnerPlugin` frame
loop). It builds the world for a named **preset** (`spaceport-aerial`, which boots the
...
`artifacts/visual/runs/` via `THALOS_SCREENSHOT_OUT`; `just compare`
uses that run tree automatically. So an agent can now see the composed
...
press **F8** to write `artifacts/diagnostics/latest_perspective.json`. When they ask
to “check my latest perspective,” run `just screenshot latest` and inspect
`artifacts/visual/latest/latest_perspective.png`. The replay preserves the body-fixed
...
take two ad-hoc manual screenshots and call them an A/B. ADR-20260721T192218Z-persistent-visual-iteration keeps one sequential,
full-resolution camera and makes the default loop persistent; a split viewport
still changes LOD, SSAO, shadows, antialiasing, and other inputs under test.
...
5. Inspect `artifacts/visual/runs/comparisons/<preset>/<axis>/` in this order:
...
7. Use the result to eliminate candidates. For bugs, do not patch until the root
   ...
   effect. After editing, rerun the exact preset/axis, then run `just compare-cold`
   for final isolated evidence and keep the matched
...
`just compare` is not a performance benchmark: its persistent caches and artifact
readback deliberately favor visual iteration over timing. Use the FPS/trace
...
get a parallel camera or orchestration path. Full details: `docs/development/visual_testing.md`.
...
under `artifacts/diagnostics/`, never beside images. Bare filename overrides for
...
> Thalos-owned `thalos_udlod` (re-exported as `thalos_body_render::udlod`). Phases
...
- **Spawn situation is a flag: ship in orbit, EVA on the
  ...
  flat land, or one of two surface-runway scenarios.**
...
- **Single-writer resources.** Frame-local projection/snapshot resources
  ...
  **Sole writer:**. Today: `SolarSystemState` ← `sync_solar_system_state`,
  ...
  ownership/warp/prediction consumer reads — see `docs/simulation/regimes.md`), and
...
`ADR-YYYYMMDDTHHMMSSZ-slug`, `INC-YYYYMMDDTHHMMSSZ-slug`) live in `docs/README.md`.** Steering docs —
...
- `regimes.md` — the per-craft `CraftRegime` resolver — one sole-writer
  ...
  resolver + Reflect-registered drift checker in `crates/runtime/game/src/regime.rs`
...
- `surface_local.md` — **implemented for ships (2026-06)**: the
  ...
  data-driven terrain-anchored **structures** (`crates/runtime/game/src/structures.rs`:
